...
if (!js.includes("function $CA()")) {
  js = js.replace(
    'JCA=[{key:"account",label:"ACCOUNT",icon:Ji.fist},{key:"codex",label:"CODEX",icon:Ji.chest},{key:"ai",label:"AI WORKER",icon:Ji.chat}]',
    `${HUB_ABOUT}JCA=[{key:"account",label:"ACCOUNT",icon:Ji.fist},{key:"about",label:"ABOUT",icon:Ji.cup},{key:"codex",label:"CODEX",icon:Ji.chest},{key:"ai",label:"AI WORKER",icon:Ji.chat}]`,
  );
}
...
const CLOUD_SYNC = `async function gO(C){try{const A=await fetch("/api/grudge/player/save?grudgeId="+encodeURIComponent(C.grudgeId),{credentials:"same-origin"});if(A.ok){const I=await A.json(),g=I?.save;g&&Object.keys(g).length&&zC.setState({onboardingDone:g.onboardingDone??!1,starterPrefabId:g.starterPrefabId??null,gbux:g.gbux??zC.getState().gbux,cards:g.cards??[],lastDailyClaim:g.lastDailyClaim??null,lastMatchReward:g.lastMatchReward??null})}const i=await fetch("/api/grudge/characters/active",{credentials:"same-origin"});if(i.ok){const e=await i.json();e?.character?.raceId&&XI.getState().setGrudgeHandoff(e.character)}}catch{}}`;

if (!js.includes("async function gO(")) {
...
js = js.replace(
  ...
  "restore:async()=>{C({loading:!0});try{const A=await kO();if(A){C({user:A,loading:!1}),await gO(A);return}const I=await cO();C({user:I,loading:!1}),I&&await gO(I)}catch{C({user:null,loading:!1})}}",
);
...
js = js.replace(
  ...
  `zC.subscribe(C=>{Fz({onboardingDone:C.onboardingDone,starterPrefabId:C.starterPrefabId,gbux:C.gbux,cards:C.cards,lastDailyClaim:C.lastDailyClaim,lastMatchReward:C.lastMatchReward});const A=Rh.getState().user?.grudgeId;A&&fetch("/api/grudge/player/save",{method:"PUT",credentials:"same-origin",headers:{"Content-Type":"application/json"},body:JSON.stringify({grudgeId:A,save:{onboardingDone:C.onboardingDone,starterPrefabId:C.starterPrefabId,gbux:C.gbux,cards:C.cards,lastDailyClaim:C.lastDailyClaim,lastMatchReward:C.lastMatchReward}})}).catch(()=>{})});`,
);
...
html = html.replace(/index-warlord-fix\d\.js/g, "index-warlord-fix3.js");
html = html.replace(/index-BNWYZMT1\.css\?v=\d+/, "index-BNWYZMT1.css?v=3");

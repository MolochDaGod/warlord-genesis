...
function clampNum(n, fallback, min = 0.01, max = 10) {
  const v = Number(n);
...
function loadPrefs() {
  try {
    ...
    if (!parsed || typeof parsed !== "object") return { ...DEFAULT, scales: { ...DEFAULT.scales }, layers: { ...DEFAULT.layers } };

    const mapSize = MAPS[parsed.mapSize] ? parsed.mapSize : DEFAULT.mapSize;
    const seed = Number.isFinite(Number(parsed.seed)) ? (Number(parsed.seed) >>> 0) : DEFAULT.seed;
    const mode = parsed.mode === "straight" || parsed.mode === "grid" ? parsed.mode : DEFAULT.mode;
    const ps = parsed.scales && typeof parsed.scales === "object" ? parsed.scales : {};
    const pl = parsed.layers && typeof parsed.layers === "object" ? parsed.layers : {};
    ...
    return {
      ...
      mode,
      scales: {
        terrain: clampNum(ps.terrain, DEFAULT.scales.terrain, 0.25, 3),
        towers: clampNum(ps.towers, DEFAULT.scales.towers, 0.15, 4),
        buildings: clampNum(ps.buildings, DEFAULT.scales.buildings, 0.15, 4),
        trees: clampNum(ps.trees, DEFAULT.scales.trees, 0.15, 4),
        props: clampNum(ps.props, DEFAULT.scales.props, 0.15, 4),
        routeLift: clampNum(ps.routeLift, DEFAULT.scales.routeLift, 0, 2.5),
      },
      layers: {
        bottom: pl.bottom !== false,
        mid: pl.mid !== false,
        top: pl.top !== false,
      },
      showNav: parsed.showNav !== false,
      showLanes: parsed.showLanes !== false,
    };
  } catch {
    return {
...
function generateBattlefield(seed, sizeKey) {
  const def = MAPS[sizeKey] || MAPS.standard;
  ...
  for (const fac of ["ally", "enemy"]) {
    ...
    for (let lane = 0; lane < 3; lane++) {
      ...
      towers.push({
        id: `tower_${fac}_${lane}_outer`,
        kind: "tower",
        x: lx,
        z: sign * length * 0.18,
        baseH: 9,
        baseW: 5.5,
        label: `${fac} outer L${lane}`,
      });
      towers.push({
        id: `tower_${fac}_${lane}_inner`,
        kind: "tower",
        x: lx * 0.55,
        z: sign * length * 0.32,
        baseH: 11,
        baseW: 6,
        label: `${fac} inner L${lane}`,
      });
      buildings.push({
        id: `bld_${fac}_barracks_${lane}`,
        kind: "building",
        x: lx - 10 * sign,
        z: sign * length * 0.36,
        baseH: 6.5,
        baseW: 8,
        label: `${fac} barracks L${lane}`,
      });
    }
  }
...
if (!canvas) {
  setStatus("ERROR: canvas #c missing");
  throw new Error("Map Edit: canvas #c not found");
}
...
function rebuildMap() {
  try {
    ...
    if (!MAPS[prefs.mapSize]) prefs.mapSize = "standard";
    if (!prefs.scales || typeof prefs.scales !== "object") prefs.scales = { ...DEFAULT.scales };
    ...
    const low = new THREE.Color(0x2d4a28);
    const high = new THREE.Color(0x6b8f4e);
    ...
    for (const st of map.structures) {
      ...
      const body = new THREE.Mesh(
        ...
        new THREE.MeshStandardMaterial({
          color: st.kind === "tower" ? 0x6b7280 : st.kind === "core" ? 0xa16207 : 0x8b7355,
          roughness: 0.85,
        }),
      );
      body.position.y = h / 2;
      ...
      for (const [lvl, t] of [
        ...
        const m = new THREE.Mesh(
          ...
          new THREE.MeshBasicMaterial({ color: COLORS[lvl] }),
        );
        m.position.y = h * t;
        g.add(m);
      }
    ...
    for (const t of map.trees) {
      ...
      const crown = new THREE.Mesh(
        ...
        new THREE.MeshStandardMaterial({ color: 0x1f6b34 }),
      );
    ...
    if (prefs.showLanes) {
      for (const lane of map.lanes) {
        const pts = lane.pts.map(
          (p) => new THREE.Vector3(p.x, map.heightAt(p.x, p.z) * s.terrain + 0.25, p.z),
        );
        root.add(
          new THREE.Line(
    ...
    fillSelects();
    setStatus(
      `Map ${prefs.mapSize} | seed ${prefs.seed.toString(16)} | structures=${map.structures.length}`,
    );
    savePrefs(prefs);
  } catch (err) {
    console.error("[map-edit] rebuildMap failed", err);
...
function fillSelects() {
  ...
  if (!from || !to || !map?.structures?.length) return;
  ...
  for (const st of map.structures) {
    ...
    from.add(new Option(label, st.id));
    to.add(new Option(label, st.id));
  }
...
function drawRoutes() {
  try {
    ...
    if (!from || !to || fromId === toId) {
      setStatus("Pick two different structures");
      return;
    }
    const s = prefs.scales;
    let xz = null;
    ...
    if (!levels.length) {
      setStatus("Enable at least one layer (bottom/mid/top)");
      return;
    }
    for (const level of levels) {
      ...
      const ground = xz.map(
        (p) => new THREE.Vector3(p.x, map.heightAt(p.x, p.z) * s.terrain + s.routeLift, p.z),
      );
    ...
    console.error("[map-edit] drawRoutes failed", err);
    setStatus(`ERROR: ${err?.message || String(err)}`);
  }
}
...
if (scalesEl) {
  for (const [key, label, min, max] of SCALE_META) {
    ...
    const val = prefs.scales?.[key] ?? DEFAULT.scales[key] ?? 1;
    wrap.innerHTML = `<div class="row"><span>${label}</span><span class="v" id="v-${key}">${Number(val).toFixed(2)}x</span></div>`;
    ...
    input.addEventListener("input", () => {
      if (!prefs.scales) prefs.scales = { ...DEFAULT.scales };
      prefs.scales[key] = Number(input.value);
      const el = document.getElementById(`v-${key}`);
      if (el) el.textContent = `${prefs.scales[key].toFixed(2)}x`;
      rebuildMap();
    });
...
if (mapSizeEl) {
  mapSizeEl.value = prefs.mapSize;
  mapSizeEl.onchange = (e) => {
    const v = e.target.value;
    prefs.mapSize = MAPS[v] ? v : "standard";
    rebuildMap();
  };
}
if (seedEl) {
  seedEl.value = prefs.seed.toString(16);
  seedEl.onchange = (e) => {
...
if (randSeed) {
  randSeed.onclick = () => {
    ...
    if (seedEl) seedEl.value = prefs.seed.toString(16);
    rebuildMap();
  };
}

const showNav = document.getElementById("showNav");
const showLanes = document.getElementById("showLanes");
if (showNav) {
  showNav.checked = prefs.showNav;
  showNav.onchange = (e) => {
    prefs.showNav = !!e.target.checked;
    rebuildMap();
  };
}
if (showLanes) {
  showLanes.checked = prefs.showLanes;
  showLanes.onchange = (e) => {
    prefs.showLanes = !!e.target.checked;
    rebuildMap();
  };
}
...
document.querySelectorAll("button.mode").forEach((btn) => {
  ...
  btn.onclick = () => {
    prefs.mode = btn.dataset.mode === "straight" ? "straight" : "grid";
    document
      .querySelectorAll("button.mode")
      .forEach((b) => b.classList.toggle("on", b.dataset.mode === prefs.mode));
    savePrefs(prefs);
  };
});
...
document.querySelectorAll("button.layer[data-layer]").forEach((btn) => {
  ...
  if (!prefs.layers) prefs.layers = { ...DEFAULT.layers };
...
if (drawBtn) drawBtn.onclick = drawRoutes;
